class Solution:
    def minDistance(self, word1: str, word2: str) -> int:

        # The code uses DP to calculate the minimum edit distance between two words.
        # Time Complexity: O(m * n), where m is the length of word1 and n is the length of word2.
        # Space Complexity: O(m * n), for the DP array.
        m, n = len(word1), len(word2)

        # Create a 2D DP array to store edit distances between substrings
        dp = [[0] * (n + 1) for _ in range(m + 1)]

        # Initialize the DP array for base cases (empty strings)
        for i in range(m + 1):
            dp[i][0] = i
        for j in range(n + 1):
            dp[0][j] = j

        # Populate the DP array using bottom-up approach
        for i in range(1, m + 1):
            for j in range(1, n + 1):
                if word1[i - 1] == word2[j - 1]:
                    dp[i][j] = dp[i - 1][j - 1]
                else:
                    dp[i][j] = min(dp[i - 1][j], dp[i][j - 1],
                                   dp[i - 1][j - 1]) + 1

        # Return the minimum edit distance (bottom-right corner of DP array)
        return dp[m][n]
